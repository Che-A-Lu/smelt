        ...
        tempFiles.map((f, i) => {
          ...
          return (
            <div key={i} style={{
              ...
              <span style={{ flex: 1, color: "#1a1a2e", fontSize: 10, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                {f.name}
              </span>
              <span style={{ color: "#9ca3af", fontSize: 9, flexShrink: 0 }}>{Math.round(f.size / 1024)}KB</span>
              <button
                onClick={() => buildCard(f)}
                style={{
                  padding: "1px 6px", border: "1px solid #e5e7eb", borderRadius: 3,
                  background: "#fafbfc", fontSize: 9, cursor: "pointer", color: "#6b7280",
                  flexShrink: 0,
                }}
              >
                {t("wb.createCard")}
